...
from .constants import BLACK, LIGHT, DARK
...
class Game(DrawnObject):
    ...
    def get_pos(self, rank: int, file: int) -> int:
    ...
    def grab_piece(self, x: int, y: int) -> None:
        ...
        piece = self.board.board[self.get_pos(rank, file)]
    ...
    def draw(self, screen: pygame.surface.Surface) -> None:
        ...
        for pos in range(120):
            ...
            # draw piece
            if piece.isalpha() and (rank, file) != self.held_piece:
                screen.blit(
                    self.images[piece],
                    (
        ...
        if self.held_piece is not None:
            ...
            screen.blit(
                self.images[self.board.board[self.get_pos(rank, file)]], (x - self.x_offset, y - self.y_offset)
            )
